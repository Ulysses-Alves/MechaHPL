	...
	*/
		...
		cVector2f vTilePos,tVertexVec* apVtxVec, cColor ShadowColor,int lFirstIndex,float fSourceSize)
	{
		...
		for(int idx =0; idx<mlShadowPointSize; idx++)
		{
			...
			mpLowLevelGraphics->AddVertexToBatch(new cVertex(vPointPos[0],ShadowColor));
			mpLowLevelGraphics->AddVertexToBatch(new cVertex(vPointPos[1],ShadowColor));
			mpLowLevelGraphics->AddIndexToBatch(lFirstIndex);
			mpLowLevelGraphics->AddIndexToBatch(lFirstIndex+1);

			mpLowLevelGraphics->AddVertexToBatch(new cVertex(vEndPos[0],ShadowColor));
			mpLowLevelGraphics->AddVertexToBatch(new cVertex(vEndPos[1],ShadowColor));
			...
			if(bExtraPos){
				mpLowLevelGraphics->AddVertexToBatch(new cVertex(vExtraPos,ShadowColor));
